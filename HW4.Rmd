...
This section contrasts observed and predicted recidivism rates given the 50% threshold, focusing on the implications for racial and racial-gender fairness.
...
And then comparing the observation and the predicted result from our model with the breakdown of race-gender groups, the plot below uncovers biases that might affect specific intersectional groups, such as Black Females, Black Male and White Males, which are the groups that are over predicted as recidivism by our model.
...
**False Negative**: The higher false negative rate for black and white female indicates a tendency to under-predict recidivism. 
...
whichThreshold %>%
  ...
  labs(title = "Figure 7: Revenue by confusion matrix type and threshold",
       y = "Revenue") +
  theme_minimal() +
  guides(colour=guide_legend(title = "Confusion Matrix")) 
```

As seen in the previous figure and in figure 8, revenue begins to normalize or flatten out at 0.75 This means that if as a department we want to maximize our budget, we should only allocate to those in the thresholds between .75 to 1.
...
whichThreshold_revenue <-
  ...
  geom_vline(xintercept =  pull(arrange(whichThreshold_revenue, -Total_Revenue)[1,1]))+
    labs(title = "Figure 8: Model Revenues By Threshold For Test Sample",
