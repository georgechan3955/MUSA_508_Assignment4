...
auc_list <- by(data = data.frame(class = testProbs_race$class, probs = testProbs_race$probs, race = testProbs_race$Race), 
               INDICES = testProbs_race$Race, 
               FUN = function(x) auc(x$class, x$probs))

# Create a data frame with the results
auc_df <- do.call(rbind, lapply(auc_list, function(x) {
  data.frame(race = names(x), AUC = x[[1]])
}))
