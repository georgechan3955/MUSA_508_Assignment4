...
We then split our data randomly into 65% of training data set and 35% of testing data set.This model shows that some indicators are significantly correlated with recidivism with p < 0.05. This include age, some categories of job, ways of contact, month last contacted, previous campaign outcome, unemployment rate, consumer price and confidence indices. We have also tried to categorize age, job number to create new variables. However, this reduces the accuracy of the model, so we would stick with the original variables.
...
whichThreshold_revenue <- 
  whichThreshold %>% 
    mutate(Recidivism = ifelse(Variable == "True_Positive", (Count * .25),
...
# Credits
ggplot(whichThreshold_revenue)+ 
  geom_line(aes(x = Threshold, y = Total_Count_Of_Recidivism))+
  geom_vline(xintercept =  pull(arrange(whichThreshold_revenue, -Total_Count_Of_Credits)[1,1]))+
    labs(title = "Figure 11: Total Count of Recidivism By Threshold",
         subtitle = "Vertical Line Denotes Optimal Threshold"))
```
