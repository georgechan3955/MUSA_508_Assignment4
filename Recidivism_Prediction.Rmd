---
title: "Recidivism Prediction"
author: "Xiayuanshan Gao, Jiahang Li, George Chen"
date: "2024-04-17"
output: 
  html_document:
    toc: true
...
In this project, we develop a model to accurately forecast the likelihood that someone convicted of a crime will commit another crime, also known as recidivism. Recidivism is a critical issue in criminal justice, with significant implications for policy-making, rehabilitation programs, and community safety. Our dependent variable would focus on recidivism within three years.

The dataset includes a variety of demographic, socio-economic, and legal variables, such as age at release, gender, race, education level, the nature of the offense, and several others. These features are important in understanding the patterns that may potentially affect individuals to re-commit crime. 

Also, exploratory data analysis, we are able to discover some initial patterns and their relationships with recidivism outcomes. Next includes with statistical testing, feature engineering, and the development of logistic regression models with a threshold at 0.3 to capture the dynamics of recidivism.

Moreover, we also include the ethical considerations of predictive modeling in criminal justice, mainly focusing on different demographic groups. By evaluating the model’s performance across these groups, we are capable to identify potential biases and ensure that our predictions do not perpetuate inequalities.
...
recidivism <- recidivism %>%
   mutate(recidivism_within_3years_numeric = case_when(
...
The dependent variable racidivism within 3 years seems to have more 'true' data than 'false' data, which would potentially add biases to the following regression model, since we are focusing on the positive true part, and it will also affect its generalization by focusing disproportionately on the positive outcomes.
...
The figures presented here display the categorical variables within our dataset. Many of the selected variables reveal varying degrees of differences in the likelihood of individuals recommitting crimes. However, while these visual differences suggest potential patterns, they are not sufficient to confirm significant differences statistically. We need to include statistical testing methods such as chi-square and ANOVA tests. These methods will allow us to determine the statistical significance of the observed differences, providing a stronger evidence for our conclusions.
...
The chi-square test is used to examine whether there's a significant association between each categorical variable and the dependent variable. Variables such as Gender, Race, Age at Release, Supervision Level First all show a very small p-value , which indicates a highly significant association with recidivism. In the Chi-square table, most of the variables we choose have siginificant association with the dependent variable.
...
## Numeric Variables
The following figures are bar charts that represents numeric variables within the dataset.
```{r cache = TRUE, warning = FALSE, message = FALSE,fig.width=10, fig.height=10}
...
The figures below display the distribution of numeric variables with respect to recidivism, our dependent variable. Similar to the categorical case, it is challenging to discern significant differences between each variable just by visual inspection. Consequently, we employed the ANOVA test to determine whether the numeric variables within the dataset have a significant association with recidivism.
...
## ANOVA TEST
For Anova table, Each row in the table corresponds to a different variable, listing its respective degrees of freedom, the sum of squares, mean squares, F value, and the associated p-value. Most of the variables are strongly associated with the recidivism except for the 'Avg Days Per Drugtest' does not seem to have a significant association.
...
Dependents: How many dependents the person have
...
Prison_offense: Type of Prison Offense
...
Residence_changes: How many times has the person changed the residence
...
The profile of an individual who is more likely to reoffend, as shaped by the logistic regression analysis, would feature a **male**, likely **white**, who is younger in age, specifically **under 48 years old at the time of release from incarceration**. The likelihood of committing another crime diminishes significantly with age, with the lowest probability of recidivism among those who are 48 or older.

This individual's past is marked by criminal behavior, with a record that may include **drug or property crime convictions**, which are associated with a higher risk of reoffending. Interestingly, while a history with one prior felony arrest seems to lower the chance of recidivism, an accumulation of misdemeanor arrests indicates a higher risk. **Not adhering to supervision condition**s also points to a greater probability of reoffending, signaling challenges with compliance to legal and corrective measures.

Stability, or the lack thereof, plays a critical role in this individual's life. **Stable employment** emerges as a significant factor that reduces the risk of reoffending, highlighting the role that consistent and meaningful work plays in rehabilitation and reintegration into society. In contrast, **frequent changes in residence** are linked to an increased risk of reoffending, which may reflect broader issues related to personal and societal stability.

Additionally, **gang affiliation** seems to be an important element of this profile. Choosing not to associate with gangs correlates with a lower likelihood of committing another crime. The predictive power of gang non-affiliation in this context could be related to the individual distancing themselves from environments and relationships that may foster criminal behavior.
...
Male,white,One delinquency report, with prior parole revocations,
Frequent residence changes increase the likelihood of reoffending (OR = 1.4772 for 3 or more changes).
...
**True Positives:** The bottom right cell (3753) represents the number of correct predictions that an outcome is the positive class ("1"). It means: predicted correctly the individuals who committed recidivism.

**False Positives: ** The top right cell (214) indicates the number of incorrect predictions where the outcome is predicted as the positive class ("1") but is actually the negative class ("0"). It means: We predicted a recidivism but the crime did not recidivate.

**True Negatives: ** The top left cell (900) indicates the number of correct predictions that an outcome is not the positive class ("0"). Here, it's the number of times the model correctly predicted the negative class. It means: predicted correctly the individuals who did not commit recidivism.

**False Negatives: ** The bottom left cell (1949) shows the number of incorrect predictions where the outcome is predicted as the negative class ("0") but is actually the positive class ("1"). It means: we predicted no recidivism but the crime did reoffend.

Our model accuracy is 0.68. This analysis prioritizes sensitivity, aiming to predict the likelihood of recidivism accurately. Our model achieves a sensitivity rate of 0.31 and a specificity of 0.95. a sensitivity of 0.31 means that 31% of individuals who do reoffend (the true positives) are correctly identified by the model. A specificity of 0.95 means that 95% of individuals who do not reoffend (the true negatives) are correctly identified by the model. This outcome is acceptable because, in this context, prioritizing true negative predictions is crucial for minimizing losses and reducing social costs.
```{r}
testProbs <- 
  testProbs %>%
...
```{r}
testProbs_race <- 
  data.frame(class = recidivismTest$recidivism_within_3years_numeric,
...
```{r}
reg.norace_gender <- glm(recidivism_within_3years_numeric ~ ., data = 
                    recidivismTrain %>% dplyr::select(recidivism_within_3years_numeric,gender,race,age_at_release,dependents, supervision_level_first,education_level,prison_years,prison_offense,violations,supervision_risk_score_first, percent_days_employed,jobs_per_year,drugtests_thc_positive,drugtests_cocaine_positive, drugtests_meth_positive,gang_affiliated,prior_arrest_episodes_felony,prior_arrest_episodes_misd,prior_arrest_episodes_property,prior_conviction_episodes_6,prior_conviction_episodes_5,prior_conviction_episodes_4,prior_conviction_episodes_3,prior_conviction_episodes_2,prior_conviction_episodes_1,prior_revocations_probation,prior_revocations_parole,delinquency_reports,residence_changes,violations_instruction),
                family = "binomial"(link = "logit"))

reg.withrace_gender <- glm(recidivism_within_3years_numeric ~ ., data = 
                      recidivismTrain %>% dplyr::select(recidivism_within_3years_numeric,gender,race,age_at_release,dependents, supervision_level_first,education_level,prison_years,prison_offense,violations,supervision_risk_score_first, percent_days_employed,jobs_per_year,drugtests_thc_positive,drugtests_cocaine_positive, drugtests_meth_positive,gang_affiliated,prior_arrest_episodes_felony,prior_arrest_episodes_misd,prior_arrest_episodes_property,prior_conviction_episodes_6,prior_conviction_episodes_5,prior_conviction_episodes_4,prior_conviction_episodes_3,prior_conviction_episodes_2,prior_conviction_episodes_1,prior_revocations_probation,prior_revocations_parole,delinquency_reports,residence_changes,violations_instruction),
                family = "binomial"(link = "logit"))
...
Based on the model, the profile of individuals more likely to commit a crime again includes those who are male, younger, with a higher number of jobs per year, a history of drug use, parole revocations, and frequent residence changes. Additionally, a history of misdemeanor arrests and certain types of prior convictions (like drug and property crimes) increase the odds of recidivism. Stable employment, non-gang affiliation, and serving longer prison sentences are associated with lower odds of reoffending. 
...
The ROC curves demonstrate the model's performance in distinguishing between recidivism outcomes for different race-gender groups. The AUC values are all relatively similar, with White Male having a marginally higher AUC, indicating the model's slightly better performance for this group. Overall, the model shows consistent discriminative ability across these categories.
...
In summary, while the model demonstrates a reasonable level of overall discriminative ability as reflected by the ROC curves, **Black Males** are subject to the model's over-prediction bias. Conversely, the model shows a better predictive accuracy for **White Male**, suggesting they are the group for whom the model performs best. This nuanced performance warrants a careful review of the model to ensure equitable outcomes across race and gender intersections.
...
In sum, the individual most likely to reoffend, as painted by the statistical model, is one who struggles with a blend of personal, legal, and socio-economic factors. Male gender, a younger age, a specific race, a history of certain types of offenses, difficulties with legal compliance, unstable employment, and frequent residential moves form the mosaic of characteristics that align with a higher propensity for recidivism. Conversely, factors such as older age, steady employment, and disassociation from gangs seem to be protective against the likelihood of reoffending.
